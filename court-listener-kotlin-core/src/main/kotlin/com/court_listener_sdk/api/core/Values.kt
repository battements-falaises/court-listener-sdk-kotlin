...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw CourtListenerInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw CourtListenerInvalidDataException("`$name` is not set")
            is JsonNull -> throw CourtListenerInvalidDataException("`$name` is null")
            else -> throw CourtListenerInvalidDataException("`$name` is invalid, received $this")
        }
